...
export async function getRecentPrompts(
  env: BigQueryEnv,
  options: { limit?: number; offset?: number; search?: string; models?: string[]; companies?: string[]; topics?: string[]; sources?: string[]; excludeSwarmIds?: string[] } = {}
...
/**
 * Delete rows from BigQuery on or after a specific date
 */
export async function deleteRowsFromDate(
  env: BigQueryEnv,
  fromDate: string
): Promise<BigQueryResult<{ deletedRows: number }>> {
  const tokenResult = await getAccessToken(env);
  ...
  const query = `
    DELETE FROM \`${env.BQ_PROJECT_ID}.${env.BQ_DATASET_ID}.${env.BQ_TABLE_ID}\`
    WHERE DATE(collected_at) >= @from_date
  `;
  ...
  try {
    ...
    const result = await response.json() as { jobComplete?: boolean; numDmlAffectedRows?: string };
